...
###########################################################
# getConnectedFolder (string pathData, string direction) : folder
###########################################################
# pathData: $PATHDATA
# direction: next|previous
# return code: 0 - ok
#              1 - last played folder not set
#              2 - requested connection not present
#
# Determine the next/previous connected folder of the last played folder
#
getConnectedFolder () {
    ...
    if [ "${DEBUG_rfid_trigger_play_sh}" == "TRUE" ]; then echo "Executing getConnectedFolder(${PATHDATA}, ${DIR})"   >> $PATHDATA/../logs/debug.log; fi

    # check last folder
    local LAST=$(cat "${PATHDATA}/../settings/Latest_Folder_Played")
    if [ -z "${LAST}" ]
    then
        if [ "${DEBUG_rfid_trigger_play_sh}" == "TRUE" ]; then echo "LASTFOLDER is not set"   >> $PATHDATA/../logs/debug.log; fi
        return 1
    fi

    if [ "${DEBUG_rfid_trigger_play_sh}" == "TRUE" ]; then echo "LASTFOLDER='${LAST}'"   >> $PATHDATA/../logs/debug.log; fi
    if [ "${DEBUG_rfid_trigger_play_sh}" == "TRUE" ]; then echo "Getting ${DIR} connected folder of '${LAST}'"   >> $PATHDATA/../logs/debug.log; fi
    local NEXT=$(${PATHDATA}/get_connected_folder.php --folder="${LAST}" --direction=${DIR})
    if [ $? -ne 0 ]
    then
        if [ "${DEBUG_rfid_trigger_play_sh}" == "TRUE" ]; then echo "Failed to get connected folder of '${LAST}'"   >> $PATHDATA/../logs/debug.log; fi
        return 2
    fi

    if [ "${DEBUG_rfid_trigger_play_sh}" == "TRUE" ]; then echo "${DIR} connected folder of ${LAST} is '${NEXT}'"   >> $PATHDATA/../logs/debug.log; fi
    echo "${NEXT}"
    return 0
}
...
if [ "$CARDID" ]; then
    ...
    case $CARDID in 
        ...
        $RECORDPLAYBACKLATEST)
            ...
            ;;
        $CMDNEXTCONNECTED)
            FOLDER=$(getConnectedFolder "${PATHDATA}" next)
            ;;
        $CMDPREVCONNECTED)
            FOLDER=$(getConnectedFolder "${PATHDATA}" previous)
            ;;
        *)
